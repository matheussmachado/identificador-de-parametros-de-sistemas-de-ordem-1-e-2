...
#define JANELA 50
#define N JANELA - 1 
#define ORDEM 1  // ordem do sistema
...
volatile double yek = 0, yek_1 = 0; // saída estimada atual e anterior
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (pode_estimar) {
      ...
      determinante = FtF[0][0] * FtF[1][1] - FtF[0][1] * FtF[1][0];
  ...
  { 
    ...
    if (identificado == true) {
      ...
      if (estimar) {        
        ...
        yek = -yek_1*theta[0][0] + uk_1*theta[1][0];
        yek_1 = yek;
...
void calcular_matriz_inversa(void)
{
  inv_FtF[0][0] = FtF[1][1] / determinante;
  inv_FtF[0][1] = -FtF[0][1] / determinante;
  inv_FtF[1][0] = -FtF[1][0] / determinante;
  inv_FtF[1][1] = FtF[0][0] / determinante;
}
